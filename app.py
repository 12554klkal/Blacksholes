...
with tab2:
    ...
    st.info("""
    **Color Interpretation for Long Option Positions:**
    * **Green:** Generally indicates a favorable sensitivity for a long option position.
    * **Red:** Generally indicates an unfavorable sensitivity or a cost for a long option position.
...
with tab3:
    ...
    st.markdown("---")
    st.subheader("Interactive Greeks Plots vs. Stock Price") # Re-added this section
        ...
        try:
            ...
            if expiries_greeks_plot:
                ...
                if selected_option_greeks_plot_str:
                    ...
                    st.info("Please select an option to view Greeks plots.")
            else:
                st.info("No option chain data available for Greeks plots.")
        except Exception as e:
            st.error(f"Error generating Greeks plots: {e}")
    else:
        st.info("Please search for a stock on the 'Black-Scholes Calculator' tab to view Greeks Analysis.")
